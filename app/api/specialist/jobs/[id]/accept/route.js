...
import { acceptJob } from "@/app/lib/specialist/accept-job.service";
...
export async function POST(req, { params }) {
  ...
  const result = await acceptJob({ jobId, specialistId });

  if (result.count === 0) {
    return NextResponse.json(
      { error: "JOB_NOT_FOUND_OR_ALREADY_ASSIGNED" },
      { status: 409 },
    );
  }
  return NextResponse.json({ code: "JOB_ACCEPTED" }, { status: 200 });
}
